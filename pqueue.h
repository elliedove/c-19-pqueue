...
private:
   ...
   ******************************************/
  void clear(Node * aNode); // Helper for clear
  void inOrder(Node* aNode) const; // Called by dump
  Node * mergeHelp(Node * aNode, Node * bNode); // Recursive merge helper
  void preOrderDump(Node * aNode) const; // Helper for printing patients in order
  void rebuild(Node * aNode); // Helper for setPriorityFn()
  Node * copy(Node * aNode); // Helper for copy constructor and assignment operator
};
